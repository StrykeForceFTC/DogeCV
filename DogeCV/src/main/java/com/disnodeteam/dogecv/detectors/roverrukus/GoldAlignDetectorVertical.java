...
public class GoldAlignDetectorVertical extends DogeCVDetector {
    ...
    public GoldAlignDetectorVertical() {
        super();
        detectorName = "Gold Align Detector Vertical";
    }
    ...
    public Mat process(Mat input) {
        ...
        if(bestRect != null){
            ...
            goldYPos = yPos;
            Imgproc.circle(workingMat, new Point( bestRect.x + ( bestRect.width / 2), yPos ), 5, new Scalar(255,0,0),2);
            ...
            Imgproc.putText(workingMat,"Current Y: " + bestRect.y,new Point(getAdjustedSize().width - 10, 10),0,0.5, new Scalar(255,255,255),1);
            found = true;
        }else{
            found = false;
            aligned = false;
        ...
        if(debugAlignment){
            ...
            Imgproc.line(workingMat,new Point(getAdjustedSize().width, alignYMin), new Point( getAdjustedSize().width - 40, alignYMin),new Scalar(255,0,0), 2);
            Imgproc.line(workingMat,new Point(getAdjustedSize().width, alignYMax), new Point(getAdjustedSize().width - 40, alignYMax),new Scalar(255,0,0), 2);
        }
        ...
        Imgproc.putText(workingMat,"Result: " + aligned,new Point(getAdjustedSize().width - 30, 10 ),0,1, new Scalar(255,255,0),1);
